...
#
# This code generates the images required for the creation of a video.
# The video was stitched together with 'convert' command on the images
# that are generated with mpeg as output type. You can vary the length
# of time that each image is displayed for (see the manual page).
#
...
#globular = 0
#def generateRGBImages(block, maxSquareSize, N, fileprefix):
#  globular = (globular + 1) % N
#  im = Image.new("RGBA", (maxSquareSize, maxSquareSize))
...
#        print block
    ...
    if is_square(len(coords)):
      ...
      if squareCount == k:
        blk = 1
        base_block = block
        generateImages(block, maxSquareSize, N, "Ond-N%d-k%d-a00" % (N, squareCount))
        for k in range(0,additers):
          for bk in base_block.keys():
            block[bk] = (block[bk] + base_block[bk]) % N 

          generateImages(block, maxSquareSize, N, "Ond-N%d-k%d-a%02d" % (N, squareCount, blk))
          blk += 1
